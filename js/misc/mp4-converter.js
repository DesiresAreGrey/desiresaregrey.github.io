...
LoadingBar.finish();
let video = null;
const inVideoPreview = $id('in-video-preview');
...
const transcodeSetting = $id('setting-transcode');
transcodeSetting.input.addEventListener('change', changedReencodeSetting);
function changedReencodeSetting() {
    const transcodingSettings = $id('transcoding-settings');
    transcodeSetting.value ? transcodingSettings.enable() : transcodingSettings.disable();
}
const presetSetting = $id('setting-preset');
const qualitySetting = $id('setting-quality');
const resolutionSetting = $id('setting-resolution');
const [widthInput, heightInput] = resolutionSetting.inputs;
const fpsSetting = $id('setting-fps');
const startEndSetting = $id('setting-start-end');
...
const outputVideo = $id('out-video-preview');
...
async function convertToMp4() {
    ...
    try {
        ...
        FFmpegHelper.onProgress = (progress) => LoadingBar.update(progress.remap(0.1, 1));
...
//# sourceMappingURL=mp4-converter.js.map