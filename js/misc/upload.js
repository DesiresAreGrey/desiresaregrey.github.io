...
async function showUploadedPopup(url, fileType) {
    const popup = await Popup.show(`
        <div class="title" style="text-align: center;">File Uploaded</div>
        <div class="preview-wrapper noselect">
            ${fileType.startsWith('video/') ? `<video id="video-preview" class="preview" src="${url}" autoplay controls playsinline disablePictureInPicture></video>` : ''}
            ${fileType.startsWith('image/') ? `<img id="image-preview" class="preview" src="${url}"/>` : ''}
        </div>
