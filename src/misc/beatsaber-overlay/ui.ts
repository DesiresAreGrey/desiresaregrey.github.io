const ui = (() => {
	...
	const beatmap = (() => {
		...
		function format(number: number) {
			...
			if (Math.abs(rounded - number) < 0.1) {
		...
		return (data, time) => {
			...
			if (data.levelAuthorName) {
				const authors = data.levelAuthorName.split(",");
				let authorText = authors[0];
				if (authors.length == 2)
					authorText += ` & ${authors[1]}`;
				else
					authorText = authors.join(", ");
				artist.innerHTML = `${data.songAuthorName} <span id="level-author">[${authorText}]</span>`;
			} else {
